...
//        disconnect(timer, SIGNAL(timeout()), this, SLOT(obfuScationStatus()));
    }
    delete ui;
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeEblockmailCoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeEblockmailCoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeEblockmailCoinAmount = strAnonymizeEblockmailCoinAmount.remove(strAnonymizeEblockmailCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeEblockmailCoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeEblockmailCoinAmount * COIN) nMaxToAnonymize = nAnonymizeEblockmailCoinAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeEblockmailCoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeEblockmailCoinAmount));
        strAnonymizeEblockmailCoinAmount = strAnonymizeEblockmailCoinAmount.remove(strAnonymizeEblockmailCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeEblockmailCoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
...
void OverviewPage::toggleObfuscation()
{
    ...
    if (!fEnableObfuscation) {
        ...
        if (nAnonymizeEblockmailCoinAmount == 0) {
