...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a EblockmailCoin address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no EblockmailCoin: URI
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert ebm:// to ebm:
    //
    //    Cannot handle this later, because ebm:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open ebm.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "EblockmailCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for EblockmailCoin.lnk
...
{
    return GetAutostartDir() / "ebm.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a ebm.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=EblockmailCoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the ebm app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add ebm app to startup item list
