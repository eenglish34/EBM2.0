...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 3434, 6082));
...
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 6082, 3539));
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (fFirstRun) {
            ...
            pwalletMain->SetBestChain(chainActive.GetLoeblockmailr());
        }
        ...
        if (GetBoolArg("-rescan", false))
            ...
            if (walletdb.ReadBestBlock(loeblockmailr))
                pindexRescan = FindForkInGlobalIndex(chainActive, loeblockmailr);
            else
                pindexRescan = chainActive.Genesis();
        }
