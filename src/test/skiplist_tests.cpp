...
BOOST_AUTO_TEST_CASE(getloeblockmailr_test)
{
    ...
    // Test 100 random starting points for loeblockmailrs.
    for (int n=0; n<100; n++) {
        int r = insecure_rand() % 150000;
        CBlockIndex* tip = (r < 100000) ? &vBlocksMain[r] : &vBlocksSide[r - 100000];
        CBlockLoeblockmailr loeblockmailr = chain.GetLoeblockmailr(tip);

        // The first result must be the block itself, the last one must be genesis.
        BOOST_CHECK(loeblockmailr.vHave.front() == tip->GetBlockHash());
        BOOST_CHECK(loeblockmailr.vHave.back() == vBlocksMain[0].GetBlockHash());

        // Entries 1 through 11 (inclusive) go back one step each.
        for (unsigned int i = 1; i < 12 && i < loeblockmailr.vHave.size() - 1; i++) {
            BOOST_CHECK_EQUAL(loeblockmailr.vHave[i].GetLow64(), tip->nHeight - i);
        }

        // The further ones (excluding the last one) go back with exponential steps.
        unsigned int dist = 2;
        for (unsigned int i = 12; i < loeblockmailr.vHave.size() - 1; i++) {
            BOOST_CHECK_EQUAL(loeblockmailr.vHave[i - 1].GetLow64() - loeblockmailr.vHave[i].GetLow64(), dist);
            dist *= 2;
        }
    }
}
